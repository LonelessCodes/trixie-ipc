...
import { NodeMessage, Server, ServerSocket } from "veza";
import { MSG_TYPE, RawMessage } from "./util";

export default class VezaServerLayer extends TranslationLayer {
    ...
    private _onMessage(message: NodeMessage, client: ServerSocket): void {
    ...
    answer(bus_wanted: string, handler: (payload: unknown) => (Promise<any> | any)): this {
        this.server.on("message", async (message: NodeMessage) => {
    ...
    async awaitAnswer(bus_request: string, payload_request?: unknown, { timeout, filter }: { timeout?: number, filter?: RegExp | string } = {}): Promise<unknown> {
        ...
        for (const [name, client] of this.server.sockets.entries()) {
